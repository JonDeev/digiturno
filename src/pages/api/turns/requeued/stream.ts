// src/pages/api/turns/requeued/stream.ts
import type { NextApiRequest, NextApiResponse } from "next";
import { prisma } from "@/lib/prisma";
import { requeuedBus } from "@/lib/requeuedBus";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const [count, svc] = await Promise.all([
    prisma.turn.count({
      where: { serviceId, status: "REQUEUED", createdAt: { gte: start, lte: end } },
    }),
  ...
  requeuedBus.on("changed", listener);

  req.on("close", () => {
    clearInterval(ping);
    requeuedBus.off("changed", listener);
    res.end();
  });
}
